...
def model_inputs():
    ...
    ph_learning_rate = tf.placeholder(tf.float32, shape = None)
    ph_keep_prob = tf.placeholder(tf.float32, shape = None, name = 'keep_prob')
    return ph_input, ph_targets, ph_learning_rate, ph_keep_prob
...
def decoding_layer_train(encoder_state, dec_cell, dec_embed_input, sequence_length, decoding_scope,
    ...
    outputs_train, _, _ = tf.contrib.seq2seq.dynamic_rnn_decoder(dec_cell, dynamic_fn_train, dec_embed_input,
            sequence_length, scope = decoding_scope)
    train_logits = output_fn(tf.nn.dropout(outputs_train,keep_prob=keep_prob))
    return train_logits
...
def decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, start_of_sequence_id, end_of_sequence_id,
    ...
    """
    # TODO: Implement Function
    dynamic_fn_infer = tf.contrib.seq2seq.simple_decoder_fn_inference(output_fn, encoder_state, dec_embeddings, start_of_sequence_id, 
...
def sentence_to_seq(sentence, vocab_to_int):
    ...
    ids_of_words = [vocab_to_int[word] if (word in vocab_to_int) else vocab_to_int['<UNK>'] for word in words]
